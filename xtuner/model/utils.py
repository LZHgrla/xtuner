...
def prepare_inputs_labels_for_multimodal_mmca(
        llm: PreTrainedModel,
    ...
    for i, (cur_new_embed, cur_new_attention_mask,
            cur_new_labels) in enumerate(
                zip(new_inputs_embeds, new_attention_mask, new_labels)):
        ...
        if cur_len > 0:
            ...
            attention_mask[i, :cur_len] = cur_new_attention_mask
...
def prepare_inputs_labels_for_multimodal(
        ...
        labels: Optional[torch.LongTensor] = None,
        pixel_values: Optional[torch.FloatTensor] = None,
        use_mmca_attn: bool = False):
    if use_mmca_attn:
        return prepare_inputs_labels_for_multimodal_mmca(
            llm, input_ids, position_ids, attention_mask, past_key_values,
            labels, pixel_values)
    if pixel_values is None:
        return {
            'input_ids': input_ids,
